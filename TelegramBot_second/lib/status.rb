...
class Status < Base
  def send_messages
    ...
    if @redis.keys("#{@user_id}-subject").empty?
      telegram_send_message("Немає лаб для здачі")
    elsif @redis.hgetall("#{@user_id}-date")["end"].nil?
      telegram_send_message("Введи інтервал семестру /semester")
    else
      telegram_send_message("Залишилось здати:")
  ...
  def date_difference_percent(date1, date2)
    ...
    if date2 >= time_now
      ...
      telegram_send_message("Інформація не актуальна.")
    end
  end
  ...
  def output_labs_count(labs_count_arr)
    ...
    telegram_send_message("Залишилось здати #{not_submited_labs_count} з #{all_labs_count} - лаб")
  end
end
